...
if __name__=="__main__":
    ...
    if len(sys.argv) != 3:
        print(f'Usage: python3 {sys.argv[0]} <Root> < Mass / SFR >')
        quit()
    ...
    for stack in range(1, Bins+1):
        ...
        for count, Split in enumerate(Splits):
            with open(f'{Directory}/MassData/{Split}/m{stack}_MassData.txt', 'r') as file:
                for line in file:
                    Tokens = line.split(', ')
            ...
            with open(f'{Directory}/SFRData/{Split}/m{stack}_SFRData.txt', 'r') as file:
                for line in file:
                    Tokens = line.split(', ')
        ...
        MassTable.write(f'{Directory}/Results/MassData/m{stack}_MassData.dat', format='ascii', overwrite=True)
        SFRTable = Table([SFRIndex, LowSFR, MedianSFR, HighSFR], names=['id', 'logSFR_low', 'logSFR', 'logSFR_High'])
        SFRTable.write(f'{Directory}/Results/SFRData/m{stack}_SFRData.dat', format='ascii', overwrite=True)
